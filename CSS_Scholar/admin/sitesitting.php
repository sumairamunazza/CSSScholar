<?php
    include("lib/session.php");
    include("lib/DBConn.php");
?>
<?php
     if(isset($_REQUEST['BtnSubmit']))
    {
        $fileinfo = PATHINFO($_FILES["image"]["name"]);
    	$newFilename = $fileinfo['filename'] ."_". time() . "." . $fileinfo['extension'];
    	move_uploaded_file($_FILES["image"]["tmp_name"],"upload/" . $newFilename);
    	$location = "upload/" . $newFilename;

        $Developer_Name = $_REQUEST['TxtName'];
        $Company = $_REQUEST['Txtcompany'];
        $Email = $_REQUEST['TxtEmail'];
        $Contact = $_REQUEST['TxtContact'];
        $Address = $_REQUEST['TxtAddress'];
        $Country = $_REQUEST['TxtCountry'];
        $Language = $_REQUEST['TxtLanguage'];
        $Facebook = $_REQUEST['TxtFB'];
        $Twitter = $_REQUEST['TxtTwitter'];
        $Google = $_REQUEST['TxtGoogle'];
       
        $Query = "INSERT INTO sitesetting ".
                   "(developername,company,email,contact,address,country, language, facebook, twitter, google) "."VALUES ".
                   "('$Developer_Name','$Company', '$Email' , '$Contact', '$Address', '$Country', '$Language', '$Facebook', '$Twitter', '$Google')";
                     $confirm_status = mysqli_query($conn,$Query);
       if($confirm_status)
       {


?>
        <script>
            alert('Record has been Successfully Inserted in Database');
            window.location.href='sitesetting.php?success';
            </script>
<?php
    }
    else
    {
        ?>
        <script type="text/javascript">alert('not Working');</script>
        <?php
    }
}
?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8" />
        <title>: : Admin Panel : :</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" />
        <meta content="A fully featured admin theme which can be used to build CRM, CMS, etc." name="description" />
        <meta content="Coderthemes" name="author" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge" />
        <link rel="shortcut icon" href="assets/images/favicon.ico">
        <link href="assets/css/bootstrap.min.css" rel="stylesheet" type="text/css" />
        <link href="assets/css/icons.css" rel="stylesheet" type="text/css" />
        <link href="assets/css/metismenu.min.css" rel="stylesheet" type="text/css" />
        <link href="assets/css/style.css" rel="stylesheet" type="text/css" />
        <script src="assets/js/modernizr.min.js"></script>
    </head>
    <body>
        <div id="wrapper">
            <?php
            include("sidebar.php");
            ?>
            </div>
            <div class="content-page">
            <?php
                include("topbar.php");
            ?>
                <!-- Start Page content -->
                <div class="content">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-lg-2"> 
                        </div>                       
                            <div class="col-lg-12">
                                <div class="card-box">
                                <center>
                                    <h4 class="header-title m-t-0">Site Setting</h4>
                                    </center>
                                    <div class="container">
                                        <form action="#" method="post" enctype="multipart/form-data">
                                            <div class="form-group">
                                                <label for="userName">Developer Name<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtName" parsley-trigger="change" required
                                                 placeholder="Developer Name" class="form-control" id="userName">
                                             </div>
                                              <div class="form-group">
                                                <label for="userName">Company Name<span class="text-danger">*</span></label>
                                                <input type="text" name="Txtcompany" parsley-trigger="change" required
                                                 placeholder="Company Name " class="form-control" id="companyName">
                                             </div>
                                             <div class="form-group">
                                                <label for="userName">Email<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtEmail" parsley-trigger="change" required
                                                 placeholder="Email" class="form-control" id="Email">
                                             </div>
                                              <div class="form-group">
                                                <label for="userName">Contact<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtContact" parsley-trigger="change" required
                                                 placeholder="Contact" class="form-control" id="contact">
                                             </div>
                                              <div class="form-group">
                                                <label for="userName">Address<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtAddress" parsley-trigger="change" required
                                                 placeholder="Address" class="form-control" id="address">
                                             </div>
                                             <div class="form-group">
                                                <label for="userName">Country<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtCountry" parsley-trigger="change" required
                                                 placeholder="Country" class="form-control" id="country">
                                             </div>
                                             <div class="form-group">
                                                <label for="userName">Language<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtLanguage" parsley-trigger="change" required
                                                 placeholder="Language" class="form-control" id="language">
                                             </div>
                                             <div class="form-group">
                                                <label for="userName">FaceBook<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtFB" parsley-trigger="change" required
                                                 placeholder="Facebook" class="form-control" id="fb">
                                             </div>
                                             <div class="form-group">
                                                <label for="userName">Twitter<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtTwitter" parsley-trigger="change" required
                                                 placeholder="Twitter" class="form-control" id="twitter">
                                             </div>
                                             <div class="form-group">
                                                <label for="userName">Google<span class="text-danger">*</span></label>
                                                <input type="text" name="TxtGoogle" parsley-trigger="change" required
                                                 placeholder="Google" class="form-control" id="google">
                                             </div>
                                                <div class="form-group text-right m-b-0">
                                                    <button class="btn btn-light waves-effect m-l-5" type="submit" name="BtnSubmit">
                                                     Submit
                                                    </button>
                                                    <button type="reset" class="btn btn-light waves-effect m-l-5">
                                                     Cancel
                                                    </button>
                                                </div>
                                         </div>
                                    </form>
                                    <br><br>
                                            
                                </div> 
                            </div>
                            <!--<div class="col-lg-3  col-offset-2"> -->
                        </div>
                 <div class="content">
                    <div class="container-fluid">
                        <div class="row">

                            <div class="col-lg-20">
                                <div class="card-box">
                                    <div class="container">
                                        <center>
                                            <h4 class="m-t-0 header-title">View Record</h4>
                                        </center>
                        <table class="table table-hover">
                                                <thead>
                                                    <tr>
                                                        <th>Developer</th>
                                                        <th>Company</th>
                                                        <th>Email</th>
                                                        <th>Contact</th>
                                                        <th>Address</th>
                                                        <th>Country</th>
                                                        <th>Language</th>
                                                        <th>FaceBook</th>
                                                        <th>Twitter</th>
                                                        <th>Google</th>
                                                        <th>Edit</th>
                                                        <th>Delete</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                        <?php
                                            $query = "SELECT * FROM sitesetting";
                                            $result = mysqli_query($conn,$query);
                                            if ($result->num_rows > 0) {            
                                            while($row = mysqli_fetch_array($result))
                                                {
                                        ?>      
                                                    <tr>
                                                        <td><?php echo $row['developername']; ?></td>
                                                        <td><?php echo $row['company']; ?></td>
                                                        <td><?php echo $row['email']; ?></td>
                                                        <td><?php echo $row['contact']; ?></td>
                                                        <td><?php echo $row['address']; ?></td>
                                                        <td><?php echo $row['country']; ?></td>
                                                        <td><?php echo $row['language']; ?></td>
                                                        <td><?php echo $row['facebook']; ?></td>
                                                        <td><?php echo $row['twitter']; ?></td>
                                                        <td><?php echo $row['google']; ?></td>
                                                        
                                                        <td><a href="sitesetting.php?id=<?php echo $row['id']; ?>"><img src="../images/edit.png" style="width:30px; height:30px;"></a>
                         
                                                        <td><a href="sitesetting.php?id=<?php echo $row['id']; ?>" 
                            onClick="return confirm('Are you sure you want to delete')"; title="Delete"><img src="../images/delete.png" style="width:30px; height:30px;"></a></td>
                                                        
                                                    </tr>
                                                   
                                                    <?php
                                                         }
                                                            }
                                                            else
                                                            {
                                                                echo "No Result Found";
                                                                  }
                                                    ?>
                                                </tbody>
                                            </table>
                        				</div>
                        			</div>
                    			</div>
                			</div>
            			</div>
       				 </div>
				</div>
            </div> 
        </div> 
                <?php
                include("includes/footer.php");
                ?>
            </div>
        </div>
        <!-- END wrapper -->
        <!-- jQuery  -->
        <script src="assets/js/jquery.min.js"></script>
        <script src="assets/js/popper.min.js"></script>
        <script src="assets/js/bootstrap.min.js"></script>
        <script src="assets/js/metisMenu.min.js"></script>
        <script src="assets/js/waves.js"></script>
        <script src="assets/js/jquery.slimscroll.js"></script>
        <!-- Parsley js -->
        <script type="text/javascript" src="../plugins/parsleyjs/parsley.min.js"></script>
        <!-- App js -->
        <script src="assets/js/jquery.core.js"></script>
        <script src="assets/js/jquery.app.js"></script>
        <script type="text/javascript">
            $(document).ready(function() {
                $('form').parsley();
            });
        </script>
    </body>
</html>